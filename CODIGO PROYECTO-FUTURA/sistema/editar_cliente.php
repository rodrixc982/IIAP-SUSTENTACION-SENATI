...
if (!empty($_POST)) {
  ...
  if (empty($_POST['nombre']) || empty($_POST['email']) || empty($_POST['telefono']) || empty($_POST['direccion']) || empty($_POST['contraseña'])) {
    ...
    if ($resul >= 1) {
      ...
      $sql_update = mysqli_query($conexion, "UPDATE cliente SET dni = $dni, nombre = '$nombre', email = '$email', telefono = '$telefono', direccion = '$direccion' WHERE idcliente = $idcliente");
        ...
        <div class="card-body">
          <form class="" action="" method="post">
            ...
            <div class="form-group">
              ...
              <input type="email" placeholder="Ingrese Correo Electrónico" name="email" class="form-control" id="email" value="<?php echo $email; ?>">
            </div>
            <div class="form-group">
